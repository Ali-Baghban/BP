...
class mcat extends xmain
{
    ...
    public function get_cat_recently()
    {
        $stmt = $this->db->prepare("SELECT * FROM {$this->tbl} WHERE status = 1 ORDER BY id DESC");
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM {$this->tbl} WHERE status = 1");
        try {
            $stmt->execute();
            $row = $stmt->fetchAll(PDO::FETCH_OBJ);
            return $row;
    ...
    public function get_holder_info($id)
    {
        $tbl = 'user_tbl';
        $tbl_2 = 'project_tbl';
        $tbl_3 = 'image_tbl';
